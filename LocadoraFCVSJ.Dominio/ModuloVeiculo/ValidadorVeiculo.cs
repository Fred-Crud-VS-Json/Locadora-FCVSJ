...
namespace LocadoraFCVSJ.Dominio.ModuloVeiculo
{
    public class ValidadorVeiculo : AbstractValidator<Veiculo>
    {
        ...
        {
            RuleFor(x => x.Modelo)
                .Matches(new Regex(@"^[.!@#$%&*a-zA-Z-0-9]{3,60}$")).WithMessage("Modelo informado é inválido. Lembre-se de preenche-lo corretamente.\n (Min 3 caracteres, Max 60 caracteres)\n (Acentos, números e espaços são permitidos)\n");
            ...
            RuleFor(x => x.Cor)
                .Matches(new Regex(@"^[ a-zA-Z-à-ü]{3,60}$")).WithMessage("Cor informada é inválida. Lembre-se de preenche-la corretamente.\n (Min 3 caracteres, Max 60 caracteres)\n (Acentos e espaços são permitidos)\n");
            ...
            RuleFor(x => x.KmPercorrido)
                .GreaterThan(0).WithMessage("Quilometragem informada é inválida. Lembre-se de preenche-la corretamente.\n");
